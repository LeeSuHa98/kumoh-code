...
public class Match
{
	...
	public void movePlay(int turn, Player p1, Player p2)
	{
		...
		while(true)
		{
			...
			if(turn == 1)
			{
				...
				// 과제 FeedBack : 오목 돌 개수가 6개 이상이 됐을 때
				// setTurn(2) 턴 넘기기
				turn = 2;
			}
			else if(turn == 2)
	...
	public void movePlay(int turn, Player p1, ComputerPlayer p2)
	{
		Scanner sc = new Scanner(System.in);
		...
		while(true)
		{
			...
			if(p1.getPlayerType() == 1)
			{
				System.out.println("유저 플레이어의 차례입니다.");
				System.out.println("------ [player " + p1.getId() + "] ------");
			...
			{
				System.out.println("컴퓨터 플레이어의 차례입니다.");
				System.out.println("------ [player " + p2.getId() + "] ------");
				
				Position position = p2.play(playBoard);
				
				if(checkValidity(position)) 
				{
					playBoard.putStone(position, StoneType.White);
					System.out.println("착수 완료.");
				}
				else 
				{
